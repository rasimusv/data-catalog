...
VALUES ((SELECT eid FROM dds.entity WHERE dds.entity.urn = 'datacatalog.airflow.etl.{{ params.exporter_name }}' LIMIT 1),
        (SELECT etid FROM  dds.entity_types WHERE entity_type_name = 'procedure' LIMIT 1),
        '{{ params.entity_name }}',
        '{{ params.entity_name_short }}',
        '{{ params.urn }}',
        (SELECT eid FROM dds.entity WHERE dds.entity.urn = '{{ params.parent_urn }}' LIMIT 1),
        'Procedure at {{ params.source_name }} {{ params.hostname }} instance',
        '{{ params.entity_name_short }}',
        '{{ params.json_data }}',
        '{{ params.json_data_ui }}',
        '{{ params.codes }}',
        ...
        NULL,
        '{"{{ params.source_name }}", "routine", "procedure"}')
  ON CONFLICT (urn)  DO UPDATE SET loaded_by_eid = (SELECT eid FROM dds.entity WHERE dds.entity.urn = 'datacatalog.airflow.etl.{{ params.exporter_name }}' LIMIT 1),
                          entity_type_etid = (SELECT etid FROM  dds.entity_types WHERE entity_type_name = 'procedure' LIMIT 1),
                          ...
                          info = 'Procedure at {{ params.source_name }} {{ params.hostname }} instance',
                          ...
                          tables = NULL,
                          tags = '{"{{ params.source_name }}", "routine", "procedure"}'
