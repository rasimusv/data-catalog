...
VALUES ((SELECT eid FROM dds.entity WHERE dds.entity.urn = 'datacatalog.airflow.etl.{{ params.exporter_name }}' LIMIT 1),
        (SELECT etid FROM  dds.entity_types WHERE entity_type_name = 'database' LIMIT 1),
        '{{ params.entity_name }}',
        '{{ params.entity_name_short }}',
        '{{ params.urn }}',
        (SELECT eid FROM dds.entity WHERE dds.entity.urn = '{{ params.parent_urn }}' LIMIT 1),
        'Schema at {{ params.source_name }} {{ params.hostname }} instance',
        REPLACE('{{ params.urn }}', '.', '   '),
        '{{ params.json_data }}',
        '{{ params.json_data_ui }}',
        NULL, NULL, NULL, NULL,
        '{{ params.tables }}',
        '{"{{ params.source_name }}", "database", "schema"}')
  ON CONFLICT (urn)  DO UPDATE SET loaded_by_eid = (SELECT eid FROM dds.entity WHERE dds.entity.urn = 'datacatalog.airflow.etl.{{ params.exporter_name }}' LIMIT 1),
                          entity_type_etid = (SELECT etid FROM  dds.entity_types WHERE entity_type_name = 'database' LIMIT 1),
                          ...
                          info = 'Schema at {{ params.source_name }} {{ params.hostname }} instance',
                          search_data = REPLACE('{{ params.urn }}', '.', '   '),
                          ...
                          tags = '{"{{ params.source_name }}", "database", "schema"}'
